...
class OhMyTasks:
    ...
    def LeerTarea(self):
        with open('tareas.txt', 'r') as pendientes:
            for tarea in pendientes.readlines():
                self.tareasPendientes.insert(tk.END, tarea.strip())
        with open('tareasRealizadas.txt', 'r') as realizadas:
            for realizada in realizadas.readlines():
                  self.tareasRealizadas.insert(tk.END, realizada.strip())
                  

    def GuardarTarea(self):
        with open('tareas.txt', 'w') as pendientes:
            for tarea in self.tareasPendientes.get(0, tk.END):
                pendientes.write(tarea + '\n')
        with open('tareasRealizadas.txt', 'w') as realizadas:
            for realizada in self.tareasRealizadas.get(0, tk.END):
                realizadas.write(realizada + '\n')
...
def main():
    root = tk.Tk()
    aplicacion = OhMyTasks(root)
    root.protocol("WM_DELETE_WINDOW", aplicacion.GuardarTarea())  # Guarda las tareas al cerrar la ventana
    root.mainloop()
main()
